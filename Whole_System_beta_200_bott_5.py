...
class EdgeNode():
    ...
    def get_tr_compressed_rep(self):
        return np.squeeze(np.array(self.tr_compressed)), self.train_data[:, -2]

    def get_val_compressed_rep(self):
        return np.squeeze(np.array(self.val_compressed)), self.val_data[:, -2]

    def get_te_compressed_rep(self):
        return np.squeeze(np.array(self.te_compressed)), self.test_data[:, -2]
...
def main():
    ...
    for kk in range(total_num_clients):
        ...
        temp1, temp2 = client_dict[str(kk)].get_tr_compressed_rep()
        temp3, temp4 = client_dict[str(kk)].get_val_compressed_rep()
        temp5, temp6 = client_dict[str(kk)].get_te_compressed_rep()
