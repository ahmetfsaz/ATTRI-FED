        ...
        with tf.variable_scope(self.clientID_str):
            ...
            with tf.variable_scope('Post_hoc_MI'):
                with tf.variable_scope('MI_U_and_Yhat_only'):
                    self.post_hoc = FCNN([1024, 1024, 256, 32], self.NUM_CLASS_PRIVATE, n_latent)
...
def Fed_Aggregation(clients, num_clients):
    ...
    weight_avg = clients[str(0)].get_post_hoc_weights()

    for kk in range(1, num_clients):
        weight_avg = np.add(weight_avg, clients[str(kk)].get_post_hoc_weights())
...
def main():
    ...
    for kk in range(total_num_clients):
        all_compressed_reps_tr.append(cr_mn['tr_x'][(len(cr_mn['tr_x']) / total_num_clients) * kk: (len(
            cr_mn['tr_x']) / total_num_clients) * (kk + 1), :])
        all_tgt_tr.append(cr_mn['tr_tgt'][
                     (len(cr_mn['tr_tgt']) / total_num_clients) * kk: (len(cr_mn['tr_tgt']) / total_num_clients) * (
                                 kk + 1), :])

        all_compressed_reps_va.append(cr_mn['val_x'][(len(cr_mn['val_x']) / total_num_clients) * kk: (len(
            cr_mn['val_x']) / total_num_clients) * (kk + 1), :])
        all_tgt_va.append(cr_mn['va_tgt'][
                     (len(cr_mn['va_tgt']) / total_num_clients) * kk: (len(cr_mn['va_tgt']) / total_num_clients) * (
                                 kk + 1), :])

        all_compressed_reps_te.append(cr_mn['te_x'][(len(cr_mn['te_x']) / total_num_clients) * kk: (len(
            cr_mn['te_x']) / total_num_clients) * (kk + 1), :])
        all_tgt_te.append(cr_mn['te_tgt'][
                     (len(cr_mn['te_tgt']) / total_num_clients) * kk: (len(cr_mn['te_tgt']) / total_num_clients) * (
                                 kk + 1), :])

    for kk in range(num_global_FL_epochs):
        for jj in range(total_num_clients):
            client_dict[str(kk)].post_hoc_adversary(all_compressed_reps_tr[jj], all_tgt_tr[jj], all_compressed_reps_va[jj],
                                                    all_tgt_va[jj], all_compressed_reps_te[jj], all_tgt_te[jj])
