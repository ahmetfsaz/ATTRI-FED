...
class EdgeNode():
        ...
        with tf.variable_scope(self.clientID_str):
            ...
            with tf.variable_scope('Post_hoc_MI'):
                with tf.variable_scope('MI_U_and_Yhat_only'):
                    MI_classifier_U_given_Yhat = FCNN([32], self.NUM_CLASS_PRIVATE, n_latent)
                    self.net_dict['private_logits_given_yhat'] = MI_classifier_U_given_Yhat(self.compressed)
    ...
    def transfer_learning_init_ADV_MNIST(self):
        ...
        self.print_plots('MNIST ', 'MNIST ', self.clientID, tr_overall_acc, va_overall_acc, te_overall_acc,
                         tr_adv_acc, va_adv_acc, te_adv_acc, tr_class_loss, va_class_loss, te_class_loss, tr_adv_loss,
                         va_adv_loss, te_adv_loss, be_va_tgt_acc, be_va_loss_CE, be_va_loss_adv, be_va_adv_acc,
                         be_te_tgt_acc, be_te_loss_CE, be_te_loss_adv, be_te_adv_acc, 100,
    ...
    def transfer_learning_init_MNIST_ADV2(self):
        ...
        for epoch_num in range(-1, num_transfer_init_adv2_epochs):
            ...
            print(
                'Client %d TRL MNIST ADV2 EPOCH %d - LR=%3.5g, W_Wt=%3.5g, Beta2_AD=%3.5g, Loss_sat=%3.5g' % (self.clientID,
                                                                                                  epoch_num,
                                                                                                  learn_rate,
                                                                                                  warmup_weight,
                                                                                                  beta2_AD,
                                                                                                  loss_sat_CE))
            if epoch_num < 0:
                print('Client %d TRL MNIST ADV2 EPOCH %d - Time taken VA:%3.5gs, TE:%3.5gs' % (self.clientID,
                                                                                                 epoch_num,
                                                                                                 va_time,
                                                                                                 te_time))
            else:
                print(
                    'Client %d TRL MNIST ADV2 EPOCH %d - Time taken TR:%3.5gs, VA:%3.5gs, TE:%3.5gs' % (self.clientID,
                                                                                             epoch_num,
                                                                                             tr_time,
                                                                                             va_time,
                                                                                             te_time))
            ...
            if best_set_CE:
                print(
                    'Client %d TRL MNIST ADV2 EPOCH %d - BE VA - ACC: %3.5g, Loss: %3.5g, CE: %3.5g, ADV_Loss: %3.5g, ROC_AUC: %3.5g, ADV_ACC: %3.5g' % (
                    self.clientID,
                    be_epoch_num_CE,
                    be_va_tgt_acc,
                    be_va_loss_GVIB,
                ...
                print(
                    'Client %d TRL MNIST ADV2 EPOCH %d - BE TE - ACC: %3.5g, Loss: %3.5g, CE: %3.5g, ADV_Loss: %3.5g, ROC_AUC: %3.5g, ADV_ACC: %3.5g' % (
                    self.clientID,
                    be_epoch_num_CE,
                    be_te_tgt_acc,
                    be_te_loss_GVIB,
    ...
    def post_hoc_adversary(self, tr_all, tr_priv, val_all, val_priv, te_all, te_priv, etr_all, etr_priv, eval_all, eval_priv, ete_all, ete_priv):
        batch_size = self.batch_size
        num_tr_iters = (2*(tr_all.shape)[0]) // batch_size
        be_va_loss_CE = np.inf
        ...
        loss_sat = 0
        be_epoch_num = -100
        for epoch_num in range(-1, num_epochs_MI_U_and_Yhat):
            temp_etrain = np.random.choice(len(etr_all), 500)
            temp_eval = np.random.choice(len(eval_all), 100)
            temp_train = np.random.choice(len(tr_all), 500)
            temp_val = np.random.choice(len(val_all), 100)
            train_combined = np.concatenate((tr_all[temp_train], etr_all[temp_etrain]), axis=0)
            val_combined = np.concatenate((val_all[temp_val], eval_all[temp_eval]), axis=0)
            test_combined = np.concatenate((te_all, ete_all), axis=0)
            tr_priv_combined = np.concatenate((tr_priv[temp_train], etr_priv[temp_etrain]), axis=0)
            vl_priv_combined = np.concatenate((val_priv[temp_val], eval_priv[temp_eval]), axis=0)
            te_priv_combined = np.concatenate((te_priv, ete_priv), axis=0)
            ...
            if epoch_num >= 0:
                ...
                for tr_iter in range(num_tr_iters):
                    ...
                    tr_all_batch = train_combined[tr_iter * batch_size:(tr_iter + 1) * batch_size, :]
                    priv_batch = tr_priv_combined[tr_iter * batch_size:(tr_iter + 1) * batch_size]
            ...
            priv_batch = vl_priv_combined

            feed_dict = {self.x: x_batch,
                         self.u: priv_batch,
                         self.v: tgt_batch,
                         self.compressed: val_combined,
                         self.learning_rate: learning_rate_py,
                         self.tf_warmup_wt: py_warmup_weight}
            va_tgt_acc = 0.0
            va_loss_CE = 0.0
            ...
            priv_batch = te_priv_combined
            tgt_batch = self.test_data[:, -1]
            feed_dict = {self.x: x_batch,
                         self.u: priv_batch,
                         self.v: tgt_batch,
                         self.compressed: test_combined,
                         self.learning_rate: learning_rate_py,
                         self.tf_warmup_wt: py_warmup_weight}
            te_tgt_acc = 0.0
            te_loss_CE = 0.0
    ...
    def save_public_compressed_rep(self):

        compressed_dict = [self.net_dict['y_hat']]

        learning_rate_py = 0.1 * 1e-3
        x_batch = self.etrain_data[:, 0:-2]
        priv_batch = self.etrain_data[:, -2]
        tgt_batch = self.etrain_data[:, -1]
        feed_dict = {self.x: x_batch,
                     self.u: priv_batch,
                     self.v: tgt_batch,
                     self.learning_rate: learning_rate_py}
        tr_op_dict = self.sess.run(compressed_dict, feed_dict=feed_dict)
        self.etr_compressed.append(tr_op_dict[0])
        print('Privatized Representations of Public TRAINING Data appended')

        x_batch = self.eval_data[:, 0:-2]
        priv_batch = self.eval_data[:, -2]
        tgt_batch = self.eval_data[:, -1]
        feed_dict = {self.x: x_batch,
                     self.u: priv_batch,
                     self.v: tgt_batch,
                     self.learning_rate: learning_rate_py}
        va_op_dict = self.sess.run(compressed_dict, feed_dict=feed_dict)
        self.eval_compressed.append(va_op_dict[0])

        print('Privatized Representations of Public VALIDATION Data appended')

        x_batch = self.etest_data[:, 0:-2]
        priv_batch = self.etest_data[:, -2]
        tgt_batch = self.etest_data[:, -1]
        feed_dict = {self.x: x_batch,
                     self.u: priv_batch,
                     self.v: tgt_batch,
                     self.learning_rate: learning_rate_py}
        ...
        te_op_dict = self.sess.run(compressed_dict, feed_dict=feed_dict)
        self.ete_compressed.append(te_op_dict[0])

        print('Privatized Representations of Public TESTING Data appended')
        return
    ...
    def get_etr_compressed_rep(self):
        return np.squeeze(np.array(self.etr_compressed)), self.etrain_data[:, -2], self.etrain_data[:, -1]

    def get_eval_compressed_rep(self):
        return np.squeeze(np.array(self.eval_compressed)), self.eval_data[:, -2], self.eval_data[:, -1]

    def get_ete_compressed_rep(self):
        return np.squeeze(np.array(self.ete_compressed)), self.etest_data[:, -2], self.etest_data[:, -1]
...
def Fed_Aggregation(clients, num_clients):
    ...
    weight_avg = clients[str(0)].get_decoder_weights()

    for kk in range(1, num_clients):
        weight_avg = np.add(weight_avg, clients[str(kk)].get_decoder_weights())

    for jj in range(len(weight_avg)):
        weight_avg[jj] = weight_avg[jj] / num_clients

    for kk in range(num_clients):
        clients[str(kk)].set_decoder_weights(weight_avg)
...
def main():
    ...
    directory = "Date11_" + current_day + "_Time_" + current_time
    parent_dir = "/storage/home/hcoda1/1/asaz3/"
    ...
    for kk in range(total_num_clients):
        ...
        client_dict[str(kk)] = EdgeNode(local_train_data[kk], local_valid_data[kk], local_test_data[kk], kk, sess,
                                        ...
                                        enum_loc_tr_datapoints, enum_loc_va_datapoints, enum_loc_te_datapoints, directory, 784*3,
    ...
    # Initial Training, do the initial transfer learning
    for kk in range(total_num_clients):
        client_dict[str(kk)].transfer_learning_phase_1a()


    # Further Transfer Learning to Strenghten the Adversary
    '''logits = []
    for kk in range(total_num_clients):
        logits.append(client_dict[str(kk)].logits_over_public_data())

    avg_logits = np.sum(np.squeeze(np.array(logits)), axis=0) / total_num_clients


    shuffle(EMN_test, avg_logits)
    tr_x_col, val_x_col, test_x_col = np.split(EMN_test, [6000, 7000])
    tr_y_col, val_y_col, test_y_col = np.split(avg_logits, [6000, 7000])


    for kk in range(total_num_clients):
        client_dict[str(kk)].collective_init_ADV_EMNIST(tr_x_col, tr_y_col, val_x_col, val_y_col, test_x_col, test_y_col)'''

    for kk in range(total_num_clients):
        client_dict[str(kk)].transfer_learning_phase_1b()

    # #Ditch the initial decoders and start with a fresh one if ditch_decoder is True
    # ditch_decoders = False
    #
    # if (ditch_decoders):
    #     fresh_decoders(client_dict, initial_dec_weights, total_num_clients)
    #
    # # Federated Learning and Aggregation
    #
    # for kk in range(num_global_FL_epochs):
    #     Fed_Learning(client_dict, total_num_clients, kk)
    #     Fed_Aggregation(client_dict, total_num_clients)

    # Testing the system with a global adversary in server

    all_compressed_reps_tr = []
    all_priv_tr = []
    all_tgt_tr = []
    all_compressed_reps_va = []
    all_priv_va = []
    all_tgt_va = []
    all_compressed_reps_te = []
    all_priv_te = []
    all_tgt_te = []


    for kk in range(total_num_clients):
        client_dict[str(kk)].save_compressed_rep()
        temp1, temp2, temp7 = client_dict[str(kk)].get_tr_compressed_rep()
        temp3, temp4, temp8 = client_dict[str(kk)].get_val_compressed_rep()
        temp5, temp6, temp9 = client_dict[str(kk)].get_te_compressed_rep()
        all_compressed_reps_tr.append(temp1)
        all_priv_tr.append(temp2)
        all_tgt_tr.append(temp7)
        all_compressed_reps_va.append(temp3)
        all_priv_va.append(temp4)
        all_tgt_va.append(temp8)
        all_compressed_reps_te.append(temp5)
        all_priv_te.append(temp6)
        all_tgt_te.append(temp9)



    all_compressed_reps_tr = np.array(all_compressed_reps_tr).reshape((total_num_clients * (local_train_data[0].shape)[0], n_latent))
    all_priv_tr = np.array(all_priv_tr).reshape((total_num_clients * (local_train_data[0].shape)[0]))
    all_tgt_tr = np.array(all_tgt_tr).reshape((total_num_clients * (local_train_data[0].shape)[0]))
    print(np.shape(all_priv_tr))
    print(np.shape(np.nonzero(all_priv_tr)))

    all_compressed_reps_va = np.array(all_compressed_reps_va).reshape((total_num_clients * (local_valid_data[0].shape)[0], n_latent))
    all_priv_va = np.array(all_priv_va).reshape((total_num_clients * (local_valid_data[0].shape)[0]))
    all_tgt_va = np.array(all_tgt_va).reshape((total_num_clients * (local_valid_data[0].shape)[0]))
    print(np.shape(all_priv_va))
    print(np.shape(np.nonzero(all_priv_va)))

    all_compressed_reps_te = np.array(all_compressed_reps_te).reshape((total_num_clients * (local_test_data[0].shape)[0], n_latent))
    all_priv_te = np.array(all_priv_te).reshape((total_num_clients * (local_test_data[0].shape)[0]))
    all_tgt_te = np.array(all_tgt_te).reshape((total_num_clients * (local_test_data[0].shape)[0]))
    print(np.shape(all_priv_te))
    print(np.shape(np.nonzero(all_priv_te)))


    # all_compressed_reps_te, all_priv_te = client_dict[str(0)].get_te_compressed_rep()
    np.savez('compressed_reps_woPub_MN_10', tr_x=all_compressed_reps_tr, tr_priv=all_priv_tr, tr_tgt=all_tgt_tr,\
                                val_x=all_compressed_reps_va, val_priv=all_priv_va, va_tgt=all_tgt_va,\
                                te_x=all_compressed_reps_te, te_priv=all_priv_te, te_tgt=all_tgt_te)
                                

    # all_compressed_reps_te, all_priv_te = client_dict[str(0)].get_te_compressed_rep()

    '''all_compressed_reps_etr = []
    all_priv_etr = []
    all_tgt_etr = []
    all_compressed_reps_eva = []
    all_priv_eva = []
    all_tgt_eva = []
    all_compressed_reps_ete = []
    all_priv_ete = []
    all_tgt_ete = []

    client_dict[str(0)].set_encoder_weights(initial_enc_weights)
    client_dict[str(0)].set_decoder_weights(initial_dec_weights)
    client_dict[str(0)].transfer_learning_init_EMNIST()
    client_dict[str(0)].save_public_compressed_rep()
    temp1, temp2, temp7 = client_dict[str(0)].get_etr_compressed_rep()
    temp3, temp4, temp8 = client_dict[str(0)].get_eval_compressed_rep()
    temp5, temp6, temp9 = client_dict[str(0)].get_ete_compressed_rep()
    all_compressed_reps_etr = temp1
    all_priv_etr = temp2
    all_tgt_etr = temp7
    all_compressed_reps_eva = temp3
    all_priv_eva = temp4
    all_tgt_eva = temp8
    all_compressed_reps_ete = temp5
    all_priv_ete = temp6
    all_tgt_ete = temp9

    print(np.shape(all_compressed_reps_te))
    print(np.shape(all_compressed_reps_ete))
    print(np.shape(all_priv_ete))
    print(np.shape(all_tgt_ete))
    print(np.shape(np.nonzero(all_priv_ete)))

    np.savez('compressed_reps_Pub_EMN_2', etr_x=all_compressed_reps_etr, etr_priv=all_priv_etr, etr_tgt=all_tgt_etr,\
                                eval_x=all_compressed_reps_eva, eval_priv=all_priv_eva, eva_tgt=all_tgt_eva,\
                                ete_x=all_compressed_reps_ete, ete_priv=all_priv_ete, ete_tgt=all_tgt_ete)'''

    client_dict[str(0)].post_hoc_adversary(all_compressed_reps_tr, all_priv_tr, all_compressed_reps_va, all_priv_va,
                                           all_compressed_reps_te, all_priv_te, all_compressed_reps_etr, all_priv_etr, all_compressed_reps_eva, all_priv_eva,
                                           all_compressed_reps_ete, all_priv_ete)
