...
def main():
    ...
    cr_mn = np.load('Only_Yashas_MNIST_10.npz')
    ...
    for kk in range(total_num_clients):
        all_compressed_reps_tr.append(cr_mn['tr_x'][int(len(cr_mn['tr_x']) / total_num_clients) * kk: int(len(cr_mn['tr_x']) / total_num_clients) * (kk + 1), :])
        all_tgt_tr.append(cr_mn['tr_tgt'][int(len(cr_mn['tr_tgt']) / total_num_clients) * kk: int(len(cr_mn['tr_tgt']) / total_num_clients) * (kk + 1)])

        all_compressed_reps_va.append(cr_mn['val_x'][int(len(cr_mn['val_x']) / total_num_clients) * kk: int(len(cr_mn['val_x']) / total_num_clients) * (kk + 1), :])
        all_tgt_va.append(cr_mn['va_tgt'][int(len(cr_mn['va_tgt']) / total_num_clients) * kk: int(len(cr_mn['va_tgt']) / total_num_clients) * (kk + 1)])

        all_compressed_reps_te.append(cr_mn['te_x'][int(len(cr_mn['te_x']) / total_num_clients) * kk: int(len(cr_mn['te_x']) / total_num_clients) * (kk + 1), :])
        all_tgt_te.append(cr_mn['te_tgt'][int(len(cr_mn['te_tgt']) / total_num_clients) * kk: int(len(cr_mn['te_tgt']) / total_num_clients) * (kk + 1)])

        print(all_compressed_reps_tr[kk].shape)
        print(all_tgt_tr[kk].shape)
        print(all_compressed_reps_va[kk].shape)
        print(all_tgt_va[kk].shape)
        print(all_compressed_reps_te[kk].shape)
        print(all_tgt_te[kk].shape)

    for kk in range(num_global_FL_epochs):
        for jj in range(total_num_clients):
            client_dict[str(jj)].post_hoc_adversary(all_compressed_reps_tr[jj], all_tgt_tr[jj], all_compressed_reps_va[jj],
