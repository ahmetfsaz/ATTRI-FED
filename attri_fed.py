"""Unified ATTRI-FED experiment runner.

This file replaces the repository's 23 copied experiment scripts with named
presets plus command-line overrides.  The neural-network and training routines
remain based on the original TensorFlow-v1-compatible implementation.

Examples:
    python attri_fed.py --list-presets
    python attri_fed.py --preset Both_Yashas_N10.py
    python attri_fed.py --preset Only_Yashas_N10.py --num-clients 6
    python attri_fed.py --preset Baseline_DP_FL_N4.py --dry-run

The ``legacy_laplace_dp`` aggregation rule reproduces the committed
Baseline_DP_FL_N4.py behavior.  It must not be described as the paper's Opacus
baseline without separately restoring and validating that implementation.

Original authors: Yashas Saidutta and Ahmet Faruk Saz.
"""
import argparse
import json
...
try:
    ...
    # Keep configuration inspection (--list-presets/--dry-run) usable on hosts
    # without the large legacy ML stack.  Actual experiments fail clearly in
    # main() before graph construction.
...
class ExperimentConfig:
    """All switches that were formerly encoded by copying the whole script."""
    ...
    latent_dim: int = 50
    beta_enforcement: float = 1.5
    beta_encouragement: float = 1.5
    beta_independence: float = 1.5
    initialization: str = "mnist_full"
    privatization: str = "local_mnist"
    collective_public_init: bool = False
    attack_input: str = "compressed"
    attack_label: str = "private"
    aggregation: str = "average"
    output_npz: Optional[str] = None
    input_npz: Optional[str] = None
    public_output_npz: Optional[str] = None
    public_input_npz: Optional[str] = None
    public_transfer_attack: bool = False
    adversarial_epochs: int = 5000
    posthoc_epochs: int = 1000
    global_fl_epochs: int = 15
    local_fl_epochs: int = 100
    seed: int = 2022
    note: str = ""


# Each historical filename remains addressable.  Exact duplicates are aliases,
# and stale/broken pipelines are represented with their original input names so
# missing artifacts fail explicitly instead of through a late NameError.
PRESETS: Dict[str, ExperimentConfig] = {
    "ADV_TEST.py": ExperimentConfig(
        "ADV_TEST.py", "public_transfer_attack", "eeo", 4, latent_dim=5,
        beta_enforcement=2.0, beta_encouragement=2.0, beta_independence=2.0,
        initialization="none", privatization="none", input_npz="Just_Us_4.npz",
        public_output_npz="Pub_EMN_4.npz", public_transfer_attack=True,
        adversarial_epochs=10000,
        note="Generates public EMNIST representations, then attacks Just_Us_4.npz.",
    ),
    "ADV_TEST-2.py": ExperimentConfig(
        "ADV_TEST-2.py", "public_transfer_attack", "eeo", 4, latent_dim=5,
        beta_enforcement=2.0, beta_encouragement=2.0, beta_independence=2.0,
        initialization="none", privatization="none", input_npz="Just_Us_4.npz",
        public_output_npz="Pub_EMN_4.npz", public_transfer_attack=True,
        adversarial_epochs=10000, note="Exact duplicate of ADV_TEST.py.",
    ),
    "ADV_TEST_only_MN.py": ExperimentConfig(
        "ADV_TEST_only_MN.py", "public_transfer_attack", "eeo", 4, latent_dim=5,
        beta_enforcement=2.0, beta_encouragement=2.0, beta_independence=2.0,
        initialization="none", privatization="none",
        input_npz="Just_Us_and_MNIST_4.npz",
        public_output_npz="Pub_EMN_4_v2.npz", public_transfer_attack=True,
        adversarial_epochs=10000,
    ),
    "Baseline_DP_FL_N4.py": ExperimentConfig(
        "Baseline_DP_FL_N4.py", "federated_current", "baseline", 4,
        initialization="mnist_target", privatization="none",
        attack_label="target", aggregation="legacy_laplace_dp",
        adversarial_epochs=10000, global_fl_epochs=50, local_fl_epochs=50,
        note="Preserves the repository's clipped/Laplace aggregator; not Opacus DP.",
    ),
    "Baseline_FL_N10.py": ExperimentConfig(
        "Baseline_FL_N10.py", "federated_current", "baseline", 10,
        initialization="mnist_target", privatization="none",
        attack_label="target", global_fl_epochs=50, local_fl_epochs=50,
        adversarial_epochs=10000,
    ),
    "Baseline_FL_N4.py": ExperimentConfig(
        "Baseline_FL_N4.py", "federated_npz", "baseline", 2,
        initialization="none", privatization="none", attack_label="target",
        input_npz="Baseline_2.npz", global_fl_epochs=50, local_fl_epochs=20,
        adversarial_epochs=10000,
        note="Historical filename says N4, but its active code uses two clients.",
    ),
    "Baseline_FL_N4_bott_1024.py": ExperimentConfig(
        "Baseline_FL_N4_bott_1024.py", "federated_current", "baseline", 4,
        latent_dim=1024, initialization="mnist_target", privatization="none",
        attack_input="raw", attack_label="target", global_fl_epochs=50,
        local_fl_epochs=50, adversarial_epochs=10000,
        note="Raw-input classifier control preserved explicitly.",
    ),
    "Baseline_N4.py": ExperimentConfig(
        "Baseline_N4.py", "privatize", "baseline", 4,
        initialization="mixed_full", privatization="none",
        collective_public_init=True, output_npz="Baseline_4.npz",
        adversarial_epochs=10000,
    ),
    "Best_of_Both_N4.py": ExperimentConfig(
        "Best_of_Both_N4.py", "privatize", "eeo", 4,
        initialization="mixed_full", privatization="local_mixed",
        collective_public_init=True, output_npz="Best_of_Both_N4.npz",
    ),
    "Beta_200_bott_5_N_10.py": ExperimentConfig(
        "Beta_200_bott_5_N_10.py", "privatize", "eeo", 10,
        latent_dim=5, beta_enforcement=2.0, beta_encouragement=2.0,
        beta_independence=2.0, initialization="mixed_full",
        privatization="local_mixed", collective_public_init=True,
        output_npz="compressed_reps_noP_10.npz", adversarial_epochs=10000,
    ),
    "Both10.py": ExperimentConfig(
        "Both10.py", "federated_npz", "eeo", 10,
        initialization="none", privatization="none", attack_label="target",
        input_npz="Both_Yashas_MNIST_10.npz", global_fl_epochs=50,
        local_fl_epochs=50, adversarial_epochs=10000,
    ),
    "Both_Yashas_N10.py": ExperimentConfig(
        "Both_Yashas_N10.py", "privatize", "eeo", 10,
        initialization="mnist_full", privatization="local_mnist",
        output_npz="Both_Yashas_MNIST_10.npz",
    ),
    "Just_Us_N10.py": ExperimentConfig(
        "Just_Us_N10.py", "privatize", "enforcement", 10,
        initialization="mixed_enforcement", privatization="local_mixed",
        collective_public_init=True, output_npz="Just_Us_10.npz",
    ),
    "Just_Us_and_MNIST_N10.py": ExperimentConfig(
        "Just_Us_and_MNIST_N10.py", "privatize", "enforcement", 10,
        initialization="mnist_enforcement", privatization="local_mnist",
        output_npz="Just_Us_and_MNIST_10.npz",
    ),
    "NoPublic_NoTransfer_ADVTEST.py": ExperimentConfig(
        "NoPublic_NoTransfer_ADVTEST.py", "precomputed_attack", "eeo", 12,
        latent_dim=5, beta_enforcement=2.0, beta_encouragement=2.0,
        beta_independence=2.0, initialization="none", privatization="none",
        input_npz="compressed_reps_woPub_MN_12.npz", adversarial_epochs=10000,
    ),
    "Only10.py": ExperimentConfig(
        "Only10.py", "federated_npz", "eco", 10,
        initialization="none", privatization="none", attack_label="target",
        input_npz="Only_Yashas_MNIST_10.npz", global_fl_epochs=50,
        local_fl_epochs=50, adversarial_epochs=10000,
    ),
    "Only_Yashas_N10.py": ExperimentConfig(
        "Only_Yashas_N10.py", "privatize", "eco", 10,
        initialization="mnist_eco", privatization="local_mnist_eco",
        output_npz="Only_Yashas_MNIST_10.npz",
    ),
    "Only_Yashas_N4.py": ExperimentConfig(
        "Only_Yashas_N4.py", "privatize", "eco", 4,
        initialization="mixed_eco", privatization="local_mixed",
        output_npz="Only_Yashas_4.npz", adversarial_epochs=10000,
    ),
    "Only_Yashas_N4-2.py": ExperimentConfig(
        "Only_Yashas_N4-2.py", "privatize", "eco", 4,
        initialization="mixed_eco", privatization="local_mixed",
        output_npz="Only_Yashas_4.npz", adversarial_epochs=5000,
        note="Differs from Only_Yashas_N4.py primarily by adversarial epochs.",
    ),
    "Pub_beta_200_bott_5_N_10.py": ExperimentConfig(
        "Pub_beta_200_bott_5_N_10.py", "privatize", "eeo", 10,
        latent_dim=5, beta_enforcement=2.0, beta_encouragement=2.0,
        beta_independence=2.0, initialization="mnist_full",
        privatization="local_mnist", output_npz="compressed_reps_woPub_MN_10.npz",
        adversarial_epochs=10000,
        note="Its public-data blocks were disabled in the committed script.",
    ),
    "Pub_beta_200_bott_5_N_10-2.py": ExperimentConfig(
        "Pub_beta_200_bott_5_N_10-2.py", "privatize", "eeo", 10,
        latent_dim=5, beta_enforcement=2.0, beta_encouragement=2.0,
        beta_independence=2.0, initialization="mixed_full",
        privatization="local_mixed", collective_public_init=True,
        output_npz="compressed_reps_Pub_MN_10.npz",
        public_output_npz="compressed_reps_Pub_EMN_10.npz",
        public_transfer_attack=True, adversarial_epochs=10000,
    ),
    "Whole_System_beta_150_bott_5.py": ExperimentConfig(
        "Whole_System_beta_150_bott_5.py", "privatize", "baseline", 12,
        latent_dim=5, initialization="mixed_full", privatization="none",
        collective_public_init=True, adversarial_epochs=10000,
        note="Despite its name, the committed phase-1b path is disabled.",
    ),
    "Whole_System_beta_200_bott_5.py": ExperimentConfig(
        "Whole_System_beta_200_bott_5.py", "privatize", "baseline", 4,
        latent_dim=5, beta_enforcement=2.0, beta_encouragement=2.0,
        beta_independence=2.0, initialization="mixed_full", privatization="none",
        collective_public_init=True, adversarial_epochs=10000,
        note="Despite its name, the committed phase-1b path is disabled.",
    ),
}
...
ACTIVE_CONFIG = PRESETS["Both_Yashas_N10.py"]
...
num_epochs_adversarial_training_enc_dec = 5000 # 250
...
num_global_FL_epochs = 15
num_local_FL_epochs = 100


def apply_config(config: ExperimentConfig) -> None:
    """Populate the legacy globals before TensorFlow graph construction."""
    global ACTIVE_CONFIG, n_latent, beta1_AD, beta2_AD, indep_AD
    global num_epochs_MI_U_and_Yhat, num_epochs_adversarial_training_enc_dec
    global num_global_FL_epochs, num_local_FL_epochs, client_dict

    ACTIVE_CONFIG = config
    n_latent = config.latent_dim
    beta1_AD = config.beta_enforcement
    beta2_AD = config.beta_encouragement
    indep_AD = config.beta_independence
    num_epochs_MI_U_and_Yhat = config.posthoc_epochs
    num_epochs_adversarial_training_enc_dec = config.adversarial_epochs
    num_global_FL_epochs = config.global_fl_epochs
    num_local_FL_epochs = config.local_fl_epochs
    client_dict = {}
...
class EdgeNode():
    ...
    def __init__(self, train_data, val_data, test_data, cl_ID, sess, NUM_CLASS_TGT, NUM_CLASS_PRIVATE, x, u, v, t, u_TFL, u_int,
        ...
        self.prior = tfc.NoisyDeepFactorized(batch_shape=(n_latent,))
        ...
        with tf.variable_scope(self.clientID_str):
            with tf.variable_scope('GVIB'):
                ...
                with tf.variable_scope('Decoder_predict_target'):
                    ...
                    # Compute rate
                    entropy_model = tfc.ContinuousBatchedEntropyModel(self.prior, coding_rank=2, compression=False)
                    _, bits = entropy_model(self.net_dict['y_hat'], training=True)
                    # Total number of bits divided by total number of pixels.
                    self.net_dict['bpp'] = tf.reduce_sum(bits) / (784 * 3.0)
            ...
            with tf.variable_scope('Post_hoc_MI'):
                with tf.variable_scope('MI_U_and_Yhat_only'):
                    post_hoc_dim = self.mnist_shape if ACTIVE_CONFIG.attack_input == "raw" else n_latent
                    post_hoc_input = self.x if ACTIVE_CONFIG.attack_input == "raw" else self.compressed
                    self.post_hoc = FCNN([32], self.NUM_CLASS_PRIVATE, post_hoc_dim)
                    self.net_dict['private_logits_given_yhat'] = self.post_hoc(post_hoc_input)
            ...
            with tf.variable_scope('GVIB_Loss'):
                ...
                utility = (1 + self.tf_warmup_wt) * self.net_dict['Loss_CE']
                enforcement = self.tf_warmup_wt * self.adv_beta * self.net_dict['Loss_P1_U_and_yhat_AD']
                encouragement = self.tf_warmup_wt * self.adv_beta2 * self.net_dict['Loss_adv2']
                rate = self.tf_warmup_wt * (self.adv_beta2 + self.indep_beta) * self.net_dict['bpp']

                if ACTIVE_CONFIG.objective == "baseline":
                    self.net_dict['Loss_GVIB'] = utility
                elif ACTIVE_CONFIG.objective == "eco":
                    self.net_dict['Loss_GVIB'] = utility + encouragement + rate
                elif ACTIVE_CONFIG.objective == "enforcement":
                    self.net_dict['Loss_GVIB'] = utility + enforcement
                elif ACTIVE_CONFIG.objective == "eeo":
                    self.net_dict['Loss_GVIB'] = utility + enforcement + encouragement + rate
                else:
                    raise ValueError("Unknown objective: %s" % ACTIVE_CONFIG.objective)
    ...
    def transfer_learning_init_MNIST(self):
        ...
        self.print_plots('MNIST ', 'MNIST ', self.clientID, tr_overall_acc, va_overall_acc,
                         te_overall_acc, tr_adv_acc, va_adv_acc, te_adv_acc, tr_class_loss, va_class_loss, te_class_loss,
                         tr_adv_loss, va_adv_loss, te_adv_loss, be_va_tgt_acc, be_va_loss_CE, be_va_loss_adv, be_va_adv_acc,
                         be_te_tgt_acc, be_te_loss_CE, be_te_loss_adv, be_te_adv_acc, 100,
    ...
    def transfer_learning_init_MNIST_ADV2(self):
        print('Starting MNIST ADV2 Training')
        print('#' * 60)
        print('#' * 60)
        print('#' * 60)
        ...
        for epoch_num in range(-1, num_transfer_init_adv2_epochs):
            ...
            if epoch_num >= 0:
                tgt_acc, adv_acc, loss_CE, loss_adv, py_warmup_weight, tr_time = \
                    self.training_fnc(train_data=self.train_data, batch_size=100,
                                      num_tr_datapoints=self.num_tr_datapoints, learning_rate_py=learn_rate,
                                      py_warmup_weight=warmup_weight, tr_dict=tr_dict, epoch_num=epoch_num,
                                      print_true=True, string_name='TRL MNIST ADV2')
                tr_overall_acc.append(tgt_acc)
                tr_adv_acc.append(adv_acc)
                tr_class_loss.append(loss_CE)
                tr_adv_loss.append(loss_adv)
            ...
            te_loss_adv, te_gvib_loss, rocauc_va, rocauc_te, va_time, te_time = \
                self.val_test_func(val_data=self.val_data, test_data=self.test_data, batch_size=100,
                                   num_va_datapoints=self.num_va_datapoints, num_te_datapoints=len(self.test_data),
                                   learning_rate_py=learn_rate, py_warmup_weight=warmup_weight, te_va_dict=te_va_dict,
                                   epoch_num=epoch_num, print_val=True, print_test=True, string_name='TRL MNIST ADV2')
        ...
        print('End of MNIST ADV2 Training')
        print('*' * 60)
        print('*' * 60)
        print('*' * 60)
        print(' ' * 60)
        ...
        self.print_plots('MNIST ADV2 ', 'MNIST ADV2 ', self.clientID, tr_overall_acc, va_overall_acc,
                         te_overall_acc, tr_adv_acc, va_adv_acc, te_adv_acc, tr_class_loss, va_class_loss, te_class_loss,
                         tr_adv_loss, va_adv_loss, te_adv_loss, be_va_tgt_acc, be_va_loss_CE, be_va_loss_adv, be_va_adv_acc,
                         be_te_tgt_acc, be_te_loss_CE, be_te_loss_adv, be_te_adv_acc, 100,
                         True, False, True, False, sub_dir)
    ...
    def local_adversarial_training_MN(self):
        print('Starting Local Adversarial Training')
        print('#' * 60)
        print('#' * 60)
        print('#' * 60)
        ...
        for epoch_num in range(-1, num_epochs_adversarial_training_enc_dec):
            ...
            if epoch_num >= 0:
                ...
                tgt_acc, _, loss_CE, _, py_warmup_weight, tr_time_1 = \
                    self.training_fnc(train_data=self.train_data, 
                                      batch_size=100,  # [epoch_num * 100:(epoch_num + 1) * 100, :]
                                      num_tr_datapoints = self.num_tr_datapoints,
                                      learning_rate_py=learn_rate_enc_dec,
                                      py_warmup_weight=warmup_weight_enc_dec, tr_dict=enc_dec_tr_dict, epoch_num=epoch_num,
                                      print_true=True, string_name='Local GAN Classification ', beta=beta_of_adv)
                tr_overall_acc.append(tgt_acc)
    ...
    def transfer_learning_phase_1a(self):
        # self.init_Log_Reg_Train()
        # self.compute_all_EMNIST_logits_and_rescale()
        # self.transfer_learning_init_EMNIST()
        # self.transfer_learning_init_ADV_EMNIST()
        # self.transfer_learning_init_MNIST()
        # self.transfer_learning_init_ADV_EMNIST()
        # self.transfer_learning_init_ADV_MNIST()
        # self.local_adversarial_training()
        # YOU MADE BETA 1.5 !!!!!!!!!!!!!!!!!
        self.transfer_learning_init_MNIST()
        self.transfer_learning_init_MNIST_ADV2()
        self.transfer_learning_init_ADV_MNIST()
        # self.local_adversarial_training_EMN_and_MN()
    ...
    def post_hoc_adversary(self, tr_all, tr_priv, val_all, val_priv, te_all, te_priv, epochs=None):
        batch_size = self.batch_size
        ...
        loss_sat = 0
        be_epoch_num = -100
        epochs = num_epochs_MI_U_and_Yhat if epochs is None else epochs
        for epoch_num in range(-1, epochs):
            ...
            if loss_sat == loss_sat_lim:
                learning_rate_py *= 0.5
                loss_sat = 0
            if learning_rate_py < 1e-7:
                print('Client %d Converged' % (self.clientID))
                break
            print('')
    ...
    def set_post_hoc_weights(self, post_hoc_weights):
        self.post_hoc.set_weights(post_hoc_weights)
...
def Fed_Aggregation(clients, num_clients, aggregation=None):
    """Aggregate the downstream classifier used for federated evaluation.

    ``legacy_laplace_dp`` intentionally reproduces Baseline_DP_FL_N4.py.  It
    clips each weight tensor to L2 norm four and adds Laplace noise to every
    client's tensor before averaging.  This is retained for historical
    reproducibility; it is not the Opacus mechanism described in the paper.
    """
    aggregation = aggregation or ACTIVE_CONFIG.aggregation
    client_weights = [clients[str(k)].get_post_hoc_weights() for k in range(num_clients)]

    if aggregation == "average":
        weight_avg = [np.array(w, copy=True) for w in client_weights[0]]
        for weights in client_weights[1:]:
            weight_avg = [a + b for a, b in zip(weight_avg, weights)]
    elif aggregation == "legacy_laplace_dp":
        threshold = 0.1
        batch_size = 100
        noise_multiplier = 1.0 / (batch_size * np.log(1.0 / threshold))
        weight_avg = [np.zeros_like(w) for w in client_weights[0]]
        for weights in client_weights:
            for index, weight in enumerate(weights):
                clipped = weight / max(1.0, np.linalg.norm(weight) / 4.0)
                noise = np.random.laplace(0.0, noise_multiplier * 4.0, weight.shape)
                weight_avg[index] += clipped + noise
    else:
        raise ValueError("Unknown aggregation rule: %s" % aggregation)

    weight_avg = [weight / float(num_clients) for weight in weight_avg]
    for kk in range(num_clients):
...
def _run_initialization(node, recipe):
    recipes = {
        "none": (),
        "mnist_target": ("transfer_learning_init_MNIST",),
        "mnist_eco": ("transfer_learning_init_MNIST", "transfer_learning_init_MNIST_ADV2"),
        "mnist_enforcement": ("transfer_learning_init_MNIST", "transfer_learning_init_ADV_MNIST"),
        "mnist_full": (
            "transfer_learning_init_MNIST",
            "transfer_learning_init_MNIST_ADV2",
            "transfer_learning_init_ADV_MNIST",
        ),
        "mixed_eco": (
            "transfer_learning_init_EMNIST_MNIST",
            "transfer_learning_init_EMNIST_MNIST",
            "transfer_learning_init_EMNIST_MNIST_ADV2",
        ),
        "mixed_enforcement": (
            "transfer_learning_init_EMNIST_MNIST",
            "transfer_learning_init_EMNIST_MNIST",
            "transfer_learning_init_ADV_EMNIST_MNIST",
        ),
        "mixed_full": (
            "transfer_learning_init_EMNIST_MNIST",
            "transfer_learning_init_EMNIST_MNIST",
    ...
    if recipe not in recipes:
        raise ValueError("Unknown initialization recipe: %s" % recipe)
    for method_name in recipes[recipe]:
        getattr(node, method_name)()


def _run_privatization(node, recipe):
    recipes = {
        "none": None,
        "local_mnist": "local_adversarial_training",
        "local_mnist_eco": "local_adversarial_training_MN",
        "local_mixed": "local_adversarial_training_EMN_and_MN",
    }
    if recipe not in recipes:
        raise ValueError("Unknown privatization recipe: %s" % recipe)
    if recipes[recipe] is not None:
        getattr(node, recipes[recipe])()


def _load_and_color_data(config):
    np.random.seed(config.seed)
    tf.set_random_seed(config.seed)

    mn_x_tr, mn_y_tr, mn_x_va, mn_y_va, mn_x_te, mn_y_te = load_normalized_Dataset("mnist", 50000)
    emn_x_tr, emn_y_tr, emn_x_va, emn_y_va, emn_x_te, emn_y_te = load_normalized_Dataset(
        "emnist/letters", 70000
    )

    mn_x_tr, mn_x_va, mn_x_te, mn_u_tr, mn_u_va, mn_u_te = Multi_Color_MNIST(
        mn_x_tr, mn_y_tr, mn_x_va, mn_y_va, mn_x_te, mn_y_te
    )
    emn_x_tr, emn_x_va, emn_x_te, emn_u_tr, emn_u_va, emn_u_te = Multi_Color_MNIST(
        emn_x_tr, emn_y_tr, emn_x_va, emn_y_va, emn_x_te, emn_y_te
    )

    mn_train, mn_val, mn_test = reshape_dataset(
        784, mn_x_tr, mn_x_va, mn_x_te, mn_y_tr, mn_y_va, mn_y_te, mn_u_tr, mn_u_va, mn_u_te
    )
    emn_train, emn_val, emn_test = reshape_dataset(
        784,
        emn_x_tr,
        emn_x_va,
        emn_x_te,
    ...
    return mn_train, mn_val, mn_test, emn_train, emn_val, emn_test


def _build_clients(config, run_dir):
    mn_train, mn_val, mn_test, emn_train, emn_val, emn_test = _load_and_color_data(config)
    if config.num_clients > config.num_data_shards:
        raise ValueError("num_clients cannot exceed num_data_shards")

    local_train = np.array_split(mn_train, config.num_data_shards)
    local_val = np.array_split(mn_val, config.num_data_shards)
    local_test = [mn_test for _ in range(config.num_data_shards)]

    sess = tf.InteractiveSession()
    x_dim = local_train[0].shape[1] - 2
    x = tf.placeholder(tf.float32, shape=(None, x_dim), name="x_input")
    u = tf.placeholder_with_default(np.ones(100, dtype=np.float32), shape=(None,), name="u_privacy")
    v = tf.placeholder(tf.float32, shape=(None,), name="v_target")
    t = tf.placeholder(tf.float32, shape=(None,), name="t_domain")
    u_tfl = tf.placeholder(tf.float32, shape=(None, 10), name="u_transfer_logits")
    compressed = tf.placeholder_with_default(
        tf.constant([list(np.zeros(n_latent))]), shape=(None, n_latent), name="compressed"
    )
    learning_rate = tf.placeholder_with_default(1e-4, shape=(), name="learning_rate")
    warmup = tf.placeholder_with_default(1.0, shape=(), name="warmup_weight")
    adv_beta = tf.placeholder_with_default(beta1_AD, shape=(), name="beta_enforcement")
    adv_beta2 = tf.placeholder_with_default(beta2_AD, shape=(), name="beta_encouragement")
    indep_beta = tf.placeholder_with_default(indep_AD, shape=(), name="beta_independence")
    u_int = tf.cast(u, tf.int32)
    v_int = tf.cast(v, tf.int32)
    t_int = tf.cast(t, tf.int32)
    u_one_hot = tf.one_hot(u_int, depth=10)

    for index in range(config.num_clients):
        tf.set_random_seed(1)
        node = EdgeNode(
            local_train[index], local_val[index], local_test[index], index, sess,
            10, 10, x, u, v, t, u_tfl, u_int, v_int, t_int, compressed,
            learning_rate, warmup, adv_beta, local_train[index].shape[0],
            local_val[index].shape[0], local_test[index].shape[0], 100, mn_test,
            emn_train, emn_val, emn_test, emn_train.shape[0], emn_val.shape[0],
            emn_test.shape[0], str(run_dir), 784 * 3, u_one_hot, adv_beta2, indep_beta,
        )
        client_dict[str(index)] = node
        node.construct_model()

    sess.run(tf.global_variables_initializer())
    reference = client_dict["0"]
    initial = {
        "decoder": copy.deepcopy(reference.get_decoder_weights()),
        "encoder": copy.deepcopy(reference.get_encoder_weights()),
        "adversary": copy.deepcopy(reference.get_adv_weights()),
        "log_reg": copy.deepcopy(reference.get_log_reg_weights()),
    }
    for index in range(1, config.num_clients):
        node = client_dict[str(index)]
        node.set_decoder_weights(initial["decoder"])
        node.set_encoder_weights(initial["encoder"])
        node.set_adv_weights(initial["adversary"])
        node.set_log_reg_weights(initial["log_reg"])

    return {
        "session": sess,
        "clients": client_dict,
        "initial": initial,
        "mnist": (mn_train, mn_val, mn_test),
        "emnist": (emn_train, emn_val, emn_test),
    }


def _encode(node, data):
    return node.sess.run(
        node.net_dict["y_hat"],
        feed_dict={node.x: data[:, :-2], node.u: data[:, -2], node.v: data[:, -1]},
    )


def _collect_private_representations(runtime, config):
    train_x, train_u, train_v = [], [], []
    val_x, val_u, val_v = [], [], []
    test_x, test_u, test_v = [], [], []
    for index in range(config.num_clients):
        node = runtime["clients"][str(index)]
        train_x.append(_encode(node, node.train_data))
        train_u.append(node.train_data[:, -2])
        train_v.append(node.train_data[:, -1])
        val_x.append(_encode(node, node.val_data))
        val_u.append(node.val_data[:, -2])
        val_v.append(node.val_data[:, -1])
        # The historical scripts evaluate every client encoder on the full test set.
        test_x.append(_encode(node, node.test_data))
        test_u.append(node.test_data[:, -2])
        test_v.append(node.test_data[:, -1])
    return {
        "tr_x": np.concatenate(train_x), "tr_priv": np.concatenate(train_u), "tr_tgt": np.concatenate(train_v),
        "val_x": np.concatenate(val_x), "val_priv": np.concatenate(val_u), "va_tgt": np.concatenate(val_v),
        "te_x": np.concatenate(test_x), "te_priv": np.concatenate(test_u), "te_tgt": np.concatenate(test_v),
    }


def _collect_public_representations(node):
    return {
        "etr_x": _encode(node, node.etrain_data),
        "etr_priv": node.etrain_data[:, -2],
        "etr_tgt": node.etrain_data[:, -1],
        "eval_x": _encode(node, node.eval_data),
        "eval_priv": node.eval_data[:, -2],
        "eva_tgt": node.eval_data[:, -1],
        "ete_x": _encode(node, node.etest_data),
        "ete_priv": node.etest_data[:, -2],
        "ete_tgt": node.etest_data[:, -1],
    }


def _artifact_output_path(run_dir, name):
    path = Path(name)
...
def _artifact_input_path(run_dir, name):
    path = Path(name)
    candidates = [path] if path.is_absolute() else [Path.cwd() / path, run_dir / path]
    for candidate in candidates:
        if candidate.exists():
            return candidate
    raise FileNotFoundError("Required representation file not found: %s" % name)


def _save_npz(run_dir, name, arrays):
    path = _artifact_output_path(run_dir, name)
    np.savez(str(path), **arrays)
    print("Saved representations to %s" % path)
    return path


def _load_npz(run_dir, name, public=False):
    path = _artifact_input_path(run_dir, name)
    with np.load(str(path)) as archive:
        arrays = {key: archive[key] for key in archive.files}
    required = (
        {"etr_x", "etr_priv", "eval_x", "eval_priv", "ete_x", "ete_priv"}
        if public else
        {"tr_x", "tr_priv", "tr_tgt", "val_x", "val_priv", "va_tgt", "te_x", "te_priv", "te_tgt"}
    )
    missing = sorted(required - set(arrays))
    if missing:
        raise KeyError("%s is missing keys: %s" % (path, ", ".join(missing)))
    return arrays


def _run_collective_public_init(runtime, config):
    logits = [np.squeeze(runtime["clients"][str(i)].logits_over_public_data()) for i in range(config.num_clients)]
    avg_logits = np.mean(np.stack(logits), axis=0)
    emn_test = runtime["emnist"][2]
    if len(emn_test) < 7001:
        raise ValueError("EMNIST test split is too small for the historical 6000/1000 split")
    tr_x, va_x, te_x = np.split(emn_test, [6000, 7000])
    tr_y, va_y, te_y = np.split(avg_logits, [6000, 7000])
    for index in range(config.num_clients):
        runtime["clients"][str(index)].collective_init_ADV_EMNIST(tr_x, tr_y, va_x, va_y, te_x, te_y)


def _attack_arrays(node, arrays, label, epochs, public_arrays=None):
    suffix = "tgt" if label == "target" else "priv"
    val_suffix = "va_tgt" if label == "target" else "val_priv"
    train_x, train_y = arrays["tr_x"], arrays["tr_" + suffix]
    val_x, val_y = arrays["val_x"], arrays[val_suffix]
    test_x, test_y = arrays["te_x"], arrays["te_" + suffix]
    if public_arrays is not None:
        train_x = np.concatenate([train_x, public_arrays["etr_x"]])
        train_y = np.concatenate([train_y, public_arrays["etr_priv"]])
        val_x = np.concatenate([val_x, public_arrays["eval_x"]])
        val_y = np.concatenate([val_y, public_arrays["eval_priv"]])
        test_x = np.concatenate([test_x, public_arrays["ete_x"]])
        test_y = np.concatenate([test_y, public_arrays["ete_priv"]])
    return node.post_hoc_adversary(train_x, train_y, val_x, val_y, test_x, test_y, epochs=epochs)


def _split_representation_archive(arrays, num_clients):
    parts = []
    for index in range(num_clients):
        parts.append({key: np.array_split(value, num_clients)[index] for key, value in arrays.items()})
    return parts


def _run_federated_from_arrays(runtime, config, arrays):
    parts = _split_representation_archive(arrays, config.num_clients)
    for round_index in range(config.global_fl_epochs):
        print("Federated round %d/%d" % (round_index + 1, config.global_fl_epochs))
        for index in range(config.num_clients):
            _attack_arrays(
                runtime["clients"][str(index)], parts[index], "target", config.local_fl_epochs
            )
        Fed_Aggregation(runtime["clients"], config.num_clients, config.aggregation)
    _attack_arrays(runtime["clients"]["0"], arrays, "target", 0)


def _run_workflow(runtime, config, run_dir):
    clients = runtime["clients"]

    if config.workflow == "federated_npz":
        arrays = _load_npz(run_dir, config.input_npz)
        _run_federated_from_arrays(runtime, config, arrays)
        return

    if config.workflow == "precomputed_attack":
        arrays = _load_npz(run_dir, config.input_npz)
        _attack_arrays(clients["0"], arrays, config.attack_label, config.posthoc_epochs)
        return

    if config.workflow == "public_transfer_attack":
        node = clients["0"]
        if config.public_input_npz:
            public_arrays = _load_npz(run_dir, config.public_input_npz, public=True)
        else:
            node.set_encoder_weights(runtime["initial"]["encoder"])
            node.set_decoder_weights(runtime["initial"]["decoder"])
            node.transfer_learning_init_EMNIST()
            public_arrays = _collect_public_representations(node)
            if config.public_output_npz:
                _save_npz(run_dir, config.public_output_npz, public_arrays)
        private_arrays = _load_npz(run_dir, config.input_npz)
        _attack_arrays(node, private_arrays, "private", config.posthoc_epochs, public_arrays)
        return

    for index in range(config.num_clients):
        _run_initialization(clients[str(index)], config.initialization)

    if config.workflow == "federated_current":
        arrays = _collect_private_representations(runtime, config)
        _run_federated_from_arrays(runtime, config, arrays)
        return

    if config.workflow != "privatize":
        raise ValueError("Unknown workflow: %s" % config.workflow)

    if config.collective_public_init:
        _run_collective_public_init(runtime, config)
    for index in range(config.num_clients):
        _run_privatization(clients[str(index)], config.privatization)

    private_arrays = _collect_private_representations(runtime, config)
    if config.output_npz:
        _save_npz(run_dir, config.output_npz, private_arrays)

    public_arrays = None
    if config.public_transfer_attack:
        node = clients["0"]
        if config.public_input_npz:
            public_arrays = _load_npz(run_dir, config.public_input_npz, public=True)
        else:
    ...
    _attack_arrays(clients["0"], private_arrays, config.attack_label, config.posthoc_epochs, public_arrays)


def _resolve_preset(name):
    if name in PRESETS:
        return PRESETS[name]
    matches = [value for key, value in PRESETS.items() if Path(key).stem == Path(name).stem]
    if len(matches) == 1:
        return matches[0]
    raise KeyError("Unknown preset %r. Use --list-presets." % name)


def _parse_args(argv=None):
    parser = argparse.ArgumentParser(
        description="Unified ATTRI-FED runner replacing the repository's 23 experiment scripts."
    )
    parser.add_argument("--preset", default="Both_Yashas_N10.py")
    parser.add_argument("--list-presets", action="store_true")
    parser.add_argument("--print-config", action="store_true")
    parser.add_argument("--dry-run", action="store_true")
    parser.add_argument("--output-dir", default="runs")
    parser.add_argument("--workflow")
    parser.add_argument("--objective", choices=["baseline", "eco", "eeo", "enforcement"])
    parser.add_argument("--num-clients", type=int)
    parser.add_argument("--num-data-shards", type=int)
    parser.add_argument("--latent-dim", type=int)
    parser.add_argument("--beta-enforcement", type=float)
    parser.add_argument("--beta-encouragement", type=float)
    parser.add_argument("--beta-independence", type=float)
    parser.add_argument("--initialization")
    parser.add_argument("--privatization")
    parser.add_argument("--attack-input", choices=["compressed", "raw"])
    parser.add_argument("--aggregation", choices=["average", "legacy_laplace_dp"])
    parser.add_argument("--input-npz")
    parser.add_argument("--output-npz")
    parser.add_argument("--public-input-npz")
    parser.add_argument("--public-output-npz")
    parser.add_argument("--adversarial-epochs", type=int)
    parser.add_argument("--posthoc-epochs", type=int)
    parser.add_argument("--global-fl-epochs", type=int)
    parser.add_argument("--local-fl-epochs", type=int)
...
def _apply_cli_overrides(config, args):
    mapping = {
        ...
        "beta_enforcement": args.beta_enforcement,
        "beta_encouragement": args.beta_encouragement,
        "beta_independence": args.beta_independence,
        "initialization": args.initialization,
        "privatization": args.privatization,
        "attack_input": args.attack_input,
        "aggregation": args.aggregation,
        "input_npz": args.input_npz,
        "output_npz": args.output_npz,
        "public_input_npz": args.public_input_npz,
        "public_output_npz": args.public_output_npz,
        "adversarial_epochs": args.adversarial_epochs,
        "posthoc_epochs": args.posthoc_epochs,
        "global_fl_epochs": args.global_fl_epochs,
        "local_fl_epochs": args.local_fl_epochs,
        "seed": args.seed,
    }
    return replace(config, **{key: value for key, value in mapping.items() if value is not None})
...
def main(argv=None):
    ...
    if args.list_presets:
        for name in sorted(PRESETS):
            cfg = PRESETS[name]
            print("%-36s workflow=%-22s objective=%-11s clients=%d" % (
                name, cfg.workflow, cfg.objective, cfg.num_clients
            ))
        return 0

    config = _apply_cli_overrides(_resolve_preset(args.preset), args)
    ...
    if _DEPENDENCY_ERROR is not None:
        raise RuntimeError(
            "Running experiments requires tensorflow, tensorflow-compression, "
            "and tensorflow-datasets. Missing dependency: %s" % _DEPENDENCY_ERROR
        )
    ...
    run_dir = artifact_dir / (Path(config.source_file).stem + "-" + stamp)
    run_dir.mkdir(parents=True, exist_ok=False)
    (run_dir / "config.json").write_text(json.dumps(asdict(config), indent=2, sort_keys=True) + "\n")
    print("Running %s in %s" % (config.source_file, run_dir))

    runtime = _build_clients(config, run_dir)
    try:
        # Stable artifact directory lets producer/consumer presets connect
        # without depending on a timestamped run name. Logs/plots stay isolated.
        _run_workflow(runtime, config, artifact_dir)
    finally:
...
if __name__ == "__main__":
    started = tm.time()
    exit_code = main()
    print("Program completed in %gs" % (tm.time() - started))
    sys.exit(exit_code)
